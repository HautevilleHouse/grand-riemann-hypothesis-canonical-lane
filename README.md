...
Canonical Lane research workspace for a flagship problem in analytic number theory and automorphic representation theory:
proving critical-line zero localization for admissible Artin and automorphic `L`-function families through an admissible spectral-transfer closure architecture.

## Main Manuscript

- [paper/GRAND_RIEMANN_HYPOTHESIS_PREPRINT.md](paper/GRAND_RIEMANN_HYPOTHESIS_PREPRINT.md)
- [paper/CANONICAL_ROUTING_INDEX.md](paper/CANONICAL_ROUTING_INDEX.md)

## Structure

- `paper/`
  - `GRAND_RIEMANN_HYPOTHESIS_PREPRINT.md`
...
1. Theorem chain first: read `paper/GRAND_RIEMANN_HYPOTHESIS_PREPRINT.md`.
...
Current GRD runner policy:

- `repro/run_repro.sh` extracts, promotes, runs `scripts/grd_closure_guard.py`, updates `repro/repro_manifest.json`, and enforces `fully_extracted` release-gate mode.
...
3. artifact key consumed by `scripts/grd_closure_guard.py`.
...
- Manuscript target: [paper/GRAND_RIEMANN_HYPOTHESIS_PREPRINT.md](paper/GRAND_RIEMANN_HYPOTHESIS_PREPRINT.md)
