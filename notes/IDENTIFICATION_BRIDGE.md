...
In-paper anchor: `paper/GRAND_RIEMANN_HYPOTHESIS_PREPRINT.md` (bridge and margin interface).

## Goal
Identify the canonical endpoint representative with the ordinary problem-side target for `proving critical-line zero localization for admissible Artin and automorphic `L`-function families through an admissible spectral-transfer closure architecture`.
...
- endpoint lock: `spectral_transfer`.
...
2. the endpoint-transfer lock `spectral_transfer` identifies the surviving representative;
3. the coherence remainder `eps_coh` is not smuggled into the endpoint;
4. the strict closure margin `M_GRD` remains positive.
...
Payload: verify `spectral_transfer` against the constants registry and runtime certificate.
...
If lock persistence, determining-class uniqueness, and strict coherence hold, then the endpoint representative is identified and the final margin `M_GRD` is meaningful.

## Current Instantiation

- endpoint-transfer artifact key: `spectral_transfer`
